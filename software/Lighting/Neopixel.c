...
void initIOs() {
    ...
    gpio_pull_down(COUNT1);
    gpio_pull_down(COUNT2);
    gpio_pull_down(COUNT3);
    gpio_pull_down(COUNT4);
    gpio_pull_down(COUNT5);
    gpio_pull_down(COUNT6);
    gpio_pull_down(COUNT7);
    gpio_pull_down(COUNT8);
    gpio_pull_down(COUNT9);
    gpio_pull_down(COUNT10);
    gpio_set_dir(COUNT1, GPIO_IN);
    gpio_set_dir(COUNT2, GPIO_IN);
    gpio_set_dir(COUNT3, GPIO_IN);
    gpio_set_dir(COUNT4, GPIO_IN);
    gpio_set_dir(COUNT5, GPIO_IN);
    gpio_set_dir(COUNT6, GPIO_IN);
    gpio_set_dir(COUNT7, GPIO_IN);
    gpio_set_dir(COUNT8, GPIO_IN);
    gpio_set_dir(COUNT9, GPIO_IN);
    gpio_set_dir(COUNT10, GPIO_IN);
}

int setLEDNums() {
    LEDnum = 0;

    LEDnum = LEDnum + gpio_get(COUNT1) + (gpio_get(COUNT2) << 1) + (gpio_get(COUNT3) << 2) + (gpio_get(COUNT4) << 3) + (gpio_get(COUNT5) << 4)
                + (gpio_get(COUNT6) << 5) + (gpio_get(COUNT7) << 6) + (gpio_get(COUNT8) << 7) + (gpio_get(COUNT9) << 8) + (gpio_get(COUNT10) << 9);
    //LEDnum = 10;
    //sleep_ms(100);
...
void NeopixelSetColorRange(uint16_t lampIdStart, uint16_t lampIdEnd, LampColor lampColor) {
    if (lampIdStart >= LEDnum) {
        return;
    }

    for (int i = lampIdStart; i <= lampIdEnd && i < LEDnum; i++) {
